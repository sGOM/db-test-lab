...
| 파일 | 내용 |
|------|------|
| [`test_schema.py`](tests/test_schema.py) | 마이그레이션 적용·멱등성, **ORM 매핑과 실제 스키마의 정합성**, 제약·인덱스 존재 |
...
| [`test_query_performance.py`](tests/test_query_performance.py) | **N+1 을 쿼리 개수로** 잡기, 인덱스를 탈 수 있는지 실행계획으로 확인, 페이징 안정성 |
...
([`dbtestlab.db.migrate()`](src/dbtestlab/db.py), 60줄). Alembic 이 필요해지면 이 함수만 갈아끼우면 된다.
